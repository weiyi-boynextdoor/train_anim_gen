...
## Flow Matching：从噪声生成未来姿态

Flow Matching（流匹配）训练生成模型，将样本从简单的噪声分布逐步变换到数据分布。神经网络学习的是一个**速度场**：给定中间样本和流时间，预测这个样本应该如何变化。生成时，通过求解常微分方程（ODE）沿着速度场前进；训练时，只需在随机采样的时间点回归已知的目标速度，无需求解整个 ODE。通用框架可参阅 [Flow Matching for Generative Modeling](https://arxiv.org/abs/2210.02747)。

在这个 Controller 中，一个生成样本是包含四个未来姿态 latent 的完整姿态块。速度场以前一姿态 `p` 和编码后的控制特征 `e` 为条件，因此生成结果取决于当前动画状态和期望行为。源噪声则为不同生成样本提供变化。

### 学习从噪声到数据的变化方向

[train_controller.py](../reference/train_controller.py) 使用直线插值路径。设 `n` 为源噪声，`y` 为数据集中的未来姿态块，`a` 为流时间：
...
这里的 `x(a)` 就是下文第一阶段中的中间姿态块 `v`。路径两端都有 `4D` 个元素。脚本通过裁剪高斯噪声、再按 `NormalizedPoseStds` 缩放来构造 `n`。

对于每个训练样本，随机采样 `a`，构造 `x(a)`，让教师根据 `[p, x(a), a, e]` 预测 `y - n`，再用均方误差计算损失。这样就得到了可以直接计算的监督信号。以一维数值为例：当 `n = -1`、`y = 3`、`a = 0.25` 时，中间值 `x(a) = 0`，目标速度为 `4`。

教师的输入不包含目标终点 `y`。它通过大量采样配对学习一个共享速度场；生成时的轨迹不必沿着某个训练配对的直线前进。

### 沿学习到的速度场生成姿态

生成蒸馏目标时，不再使用数据集中的未来终点。从新采样的噪声出发，对教师预测的速度做数值积分；生成同一个姿态块期间，保持 `p` 和 `e` 不变：
...
这就是脚本使用的显式 Euler 积分：每步将流时间推进 `1 / S`，并将当前速度乘以这个步长后加到 `x` 上。执行 `S` 步后，`x` 近似为流时间 `1` 处生成的未来姿态块。

**流时间与动画时间是两个不同的概念。** `a = 0 ... 1` 描述的是噪声变成一个姿态块的过程；姿态块中的四个条目对应动画中偏移 `1、2、4、8` 帧的姿态。预测速度表示 latent 坐标相对于流时间的变化率，不是关节速度或根运动速度。

虽然网络名为 `denoiser_network`，它预测的是上述速度，而不是源噪声或最终姿态。第一阶段对每个样本只在一个随机时间点执行教师计算，学习速度场；第二阶段多次执行教师并积分生成目标，让每个 LOD 学会用一次前向计算近似整个从噪声到姿态的映射。运行时则使用前文所述的某个选定 LOD。
